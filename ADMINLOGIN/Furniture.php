...
if(isset($_POST['submit'])){ 
  $c_id=3;
  if(!empty($_POST['fname']) && !empty($_POST['email']) && !empty($_POST['dop']) && !empty($_POST['price'])  && !empty($_POST['option'])) {  
          $fname=$_POST['fname'];
          $email=$_POST['email'];  
        $dop=$_POST['dop']; 
        $price=$_POST['price']; 
    ...
    $sql = $pdo->prepare("insert into furniture(f_name,v_id,dop,price,c_id,master) values(:fname,:v_id,:dop,:price,:c_id,:option)");
    ...
    $sql->execute();
     ...
     echo '</script>';
     echo '<meta http-equiv="refresh" content="0;url= Furniture.php" />';
